...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into Users (userName,nickName,userPassword,email,phoneNum,profileImgUrl) VALUES (?,?,?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getUserName(),postUserReq.getNickName(),postUserReq.getUserPassword(),postUserReq.getEmail(),postUserReq.getPhoneNum(),postUserReq.getProfileImgUrl()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public int checkPhoneNum(String phoneNum) {
        String checkPhoneNumQuery = "select exists(select phoneNum from Users where phoneNum = ?)"; // User Table에 해당 email 값을 갖는 유저 정보가 존재하는가?
    ...
    public int modifyUserName(PatchUserReq patchUserReq) {
        String modifyUserNameQuery = "update Users set nickName = ?, updateAt=CURRENT_TIMESTAMP() where userIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select * from Users where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
        ...
        return this.jdbcTemplate.queryForObject(getPwdQuery,
                (rs, rowNum) -> new User(
                        ...
                        rs.getString("mailFlag"),
                        rs.getString("smsFlag"),
    ...
    public List<GetUserRes> getUsers() {
        String getUsersQuery = "select * from Users"; //User 테이블에 존재하는 모든 회원들의 정보를 조회하는 쿼리
        return this.jdbcTemplate.query(getUsersQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getString("userStatus")) // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
        ); // 복수개의 회원정보들을 얻기 위해 jdbcTemplate 함수(Query, 객체 매핑 정보)의 결과 반환(동적쿼리가 아니므로 Parmas부분이 없음)
    }

    // 해당 nickname을 갖는 유저들의 정보 조회
    public List<GetUserRes> getUsersByNickname(String nickname) {
        String getUsersByNicknameQuery = "select * from Users where nickname =?"; // 해당 이메일을 만족하는 유저를 조회하는 쿼리문
    ...
    public GetUserRes getUser(int userIdx) {
        String getUserQuery = "select * from Users where userIdx = ?"; // 해당 userIdx를 만족하는 유저를 조회하는 쿼리문
    ...
    public int checkUserStatus(int userIdx) {
        String checkStatusQuery = "select exists(select userStatus from Users where userIdx = ?)";
        int checkStatusParams = userIdx;
    ...
    public int deleteUser(DeleteUserReq deleteUserReq) {
        String modifyUserQuery = "update Users set updateAt=CURRENT_TIMESTAMP(),userStatus='withdraw' where userIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
        int modifyUserParams = deleteUserReq.getUserIdx();
        return this.jdbcTemplate.update(modifyUserQuery, modifyUserParams); // 대응시켜 매핑시켜 쿼리 요청(생성했으면 1, 실패했으면 0)
    }

    public User getPwd(DeleteUserReq deleteUserReq) {
        String getPwdQuery = "select * from Users where userIdx = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
        int getPwdParams = deleteUserReq.getUserIdx(); // 주입될 email값을 클라이언트의 요청에서 주어진 정보를 통해 가져온다.
