...
public class UserController {
    ...
    public BaseResponse<PostUserRes> createUser(@RequestBody PostUserReq postUserReq) {
        ...
        if(!isRegexNickName(postUserReq.getNickName())){
            return new BaseResponse<>(USERS_INVALID_USER_NICKNAME);
        }
        //비밀번호 형식이 맞는지 확인 (소문자, 숫자, 특수기호 하나이상, 8-20자)
        if (!isRegexPassword(postUserReq.getUserPassword())) {
    ...
    public BaseResponse<PostLoginRes> logIn(@RequestBody PostLoginReq postLoginReq) {
        try {
            ...
            //비밀번호 형식이 맞는지 확인 (소문자, 숫자, 특수기호 하나이상, 8-20자)
            if (!isRegexPassword(postLoginReq.getUserPassword())) {
                return new BaseResponse<>(POST_INVALID_PASSWORD);
            }
    ...
    public BaseResponse<List<GetUserRes>> getUsers(@RequestParam(required = false) String nickname) {
        ...
        try {
            ...
            //null값이면 메시지를 반환한다.
            if(getUsersRes.isEmpty()){
                String message =nickname+"라는 닉네임을 가진 유저가 존재하지 않습니다.";
    ...
    public BaseResponse<String> modifyUserName(@PathVariable("userIdx") int userIdx, @RequestBody User user) {
        try {
            ...
            if(nickName!=null && isRegexNickName(nickName)) {
    ...
    public BaseResponse<String> withDrawUser(@PathVariable("userIdx") int userIdx, @RequestBody User user) {
        try {
            ...
            if(userPassword!=null && isRegexPassword(userPassword)) {
                System.out.println("userPassword = " + userPassword);
                DeleteUserReq deleteUserReq = new DeleteUserReq(userIdx,userPassword);
                userService.deleteUser(deleteUserReq);
            }

            String result = "회원삭제가 완료되었습니다.";
            return new BaseResponse<>(result);
        } catch (BaseException exception) {
            return new BaseResponse<>((exception.getStatus()));
        }
