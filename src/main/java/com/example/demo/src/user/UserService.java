...
public class UserService {
    ...
    public PostUserRes createUser(PostUserReq postUserReq) throws BaseException {
        ...
        if (userProvider.checkEmail(postUserReq.getPhoneNum()) == 1) {
        ...
        try {
            ...
            pwd = new AES128(Secret.USER_INFO_PASSWORD_KEY).encrypt(postUserReq.getUserPassword()); // 암호화코드
...
//  *********************************************************************
    ...
    public void deleteUser(DeleteUserReq deleteUserReq) throws BaseException {
        //active 상태일때만 가능
        if (userProvider.checkUserStatus(deleteUserReq.getUserIdx())==0){
            throw new BaseException(USER_STATUS_ERROR);
        }
        //비밀번호가 맞는지 확인
        if(userProvider.checkPassword(deleteUserReq)==1){
            throw new BaseException(POST_INVALID_PASSWORD);
        }
        try{
